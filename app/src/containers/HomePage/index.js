...
class HomePage extends Component {
  ...
  onContactClick(index) {
    const renderedContacts = contactData.filter(contact => {
       return !isNull(contact.phoneNumbers) && !isUndefined(contact.phoneNumbers) && contact.phoneNumbers.length > 0 && (contact.displayName.indexOf(this.state.searchKey) != -1 ||
        contact.phoneNumbers[0].value.indexOf(this.state.searchKey) != -1);
    })
    this.inputRef[this.state.currentPhoneBook].current.value = renderedContacts[index].phoneNumbers[0].value
    this.setState({
      searchKey: '',
      showPhoneBookDialog: false,
      currentPhoneBook: -1
